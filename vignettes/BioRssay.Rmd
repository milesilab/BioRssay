...
The package also allows computing the lethal dose for 25%, 50% and 95% of the populations (LD25, LD50 and LD95 respectively) or the level as specified by the user, with their 95% confidence intervals (CI) and variance of each (e.g., LD25var, LD50var, etc.), following Johnson et al. 2013 approach, which allows taking the heterogeneity of the data into account (*Finney 1971*) to calculate the CI (i.e. a larger heterogeneity will increase the CI).

The methods implemented here use a likelihood ratio test (LRT) to test for differences in resistance levels among different strains.  Finally, resistance ratios (RR) at LD25, LD50 and LD95, i.e. the ratios between a given strain and the strain with the lowest LD50 and a given LD value (e.g. LD25,LD50, and LD95); usually it is the susceptible reference, with their 95% confidence intervals, are calculated according to Robertson and *Preisler (1992)*.
...
*If many strains are present and only one (few) fails the linearity tests, we do recommend users to remove such specific strains from the analyses.*
